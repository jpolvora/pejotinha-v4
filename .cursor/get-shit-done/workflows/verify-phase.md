...
<required_reading>
@L:/source/pejotinha-v4/.cursor/get-shit-done/references/verification-patterns.md
@L:/source/pejotinha-v4/.cursor/get-shit-done/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(node "L:/source/pejotinha-v4/.cursor/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
node "L:/source/pejotinha-v4/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "L:/source/pejotinha-v4/.cursor/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "L:/source/pejotinha-v4/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "L:/source/pejotinha-v4/.cursor/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "L:/source/pejotinha-v4/.cursor/get-shit-done/bin/gsd-tools.cjs" verify key-links "$plan")
...
See L:/source/pejotinha-v4/.cursor/get-shit-done/templates/verification-report.md for complete template.
</step>
