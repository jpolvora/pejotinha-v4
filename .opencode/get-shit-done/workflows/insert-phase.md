...
```bash
INIT=$(node "L:/source/pejotinha-v4/.opencode/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "L:/source/pejotinha-v4/.opencode/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
