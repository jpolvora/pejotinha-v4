...
- **Global scope**: `L:/source/pejotinha-v4/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `L:/source/pejotinha-v4/.opencode/notes/`)
...
2. Glob `L:/source/pejotinha-v4/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (L:/source/pejotinha-v4/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `L:/source/pejotinha-v4/.opencode/notes/` — works in any directory
